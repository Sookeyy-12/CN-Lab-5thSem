...
int main() {
    ...
    int greatest;
    int isRecv = recvfrom(sockfd, &greatest, sizeof(greatest), 0, (struct sockaddr*)&clientAddr, &clientAddrLen);
    if (isRecv < 0) printf("Failed to Receive\n");
    else printf("Greatest Recieved: %d\n", greatest);
